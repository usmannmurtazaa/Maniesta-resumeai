      ...
      <div className="relative z-10 flex min-h-screen">
        ...
        >
          <div className="flex items-center justify-between border-b border-white/40 p-4">
            {!collapsed && (
              <span className="font-display text-lg font-semibold text-gray-900">Admin</span>
            )}
            <motion.button
              whileTap={prefersReducedMotion ? {} : { scale: 0.9 }}
              whileHover={prefersReducedMotion ? {} : { scale: 1.1 }}
              onClick={() => setCollapsed(!collapsed)}
              className="rounded-lg p-1 text-gray-500 hover:bg-white/60 hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary-500"
              aria-label={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}
            >
              <ChevronIcon
                size={20}
                className={`transform ${collapsed ? 'rotate-180' : ''} transition-transform duration-300`}
              />
            </motion.button>
          </div>
          <nav className="flex-1 space-y-1 py-4 px-2">
            {adminNavItems.map((item) => (
              ...
              >
                {({ isActive }) => (
                  <motion.span
                    whileHover={prefersReducedMotion ? {} : { x: collapsed ? 0 : 4 }}
